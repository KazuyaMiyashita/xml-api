...
// @ts-ignore
import { ModelElement, ModelNodeType, ModelNode } from '@miy2/xml-api/model/xml-api-model';
// @ts-ignore
...
const WYSIWYGEditor: React.FC<WYSIWYGEditorProps> = ({ api, onExternalChange }) => {
  ...
  const pmToXml = (pmDoc: PMNode): string => {
    ...
    if (bodyMatch) {
      const before = currentSource.substring(0, bodyMatch.index! + bodyMatch[1].length);
      const after = currentSource.substring(bodyMatch.index! + bodyMatch[0].length - bodyMatch[3].length);
  ...
  useEffect(() => {
    ...
    const updateInitialState = () => {
      ...
      if (docModel && viewRef.current) {
        ...
        const pmDoc = PMDOMParser.fromSchema(xhtmlSubsetSchema).parse(tempDom as any);
    ...
    const state = EditorState.create({
      doc: xhtmlSubsetSchema.node("doc", null, [xhtmlSubsetSchema.node("paragraph", null, [xhtmlSubsetSchema.text("Loading...")])]),
  ...
  return (
    <div className="wysiwyg-container">
      <div className="wysiwyg-toolbar">
        ...
        }} onMouseDown={(e) => e.preventDefault()}>Bold</button>
      </div>
