...
export function convert(node: CST, input: string): AST | string | null {
    ...
    if (node.name === 'element' || node.name === 'document') {
        ...
        if (structural.children.length === 3 && structural.children[0].name === 'STag') {
            ...
            if (initialCharDataRep && initialCharDataRep.children.length > 0) {
                 elem.children.push(convert(initialCharDataRep.children[0], input) as string);
            }
            ...
            if (repeatingPart) {
                for (const seqNode of repeatingPart.children) {
                    ...
                    if (trailingCharDataRep && trailingCharDataRep.children.length > 0) {
                         elem.children.push(convert(trailingCharDataRep.children[0], input) as string);
                    }
                }
            }
            return elem;
        ...
        // Case 2: EmptyElemTag
        if (structural.children.length >= 2 && 
    ...
    if (node.children.length === 1 && node.children[0].start === node.start && node.children[0].end === node.end) {
