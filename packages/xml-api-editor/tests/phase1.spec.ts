...
test.describe('Phase 1: Foundation', () => {
  ...
  test('Setup Basic Split-Pane UI', async ({ page }) => {
    ...
    // Check pane headers
    await expect(page.locator('.pane-left .pane-header').first()).toContainText('WYSIWYG Editor');
    await expect(page.locator('.pane-right .pane-header').first()).toContainText('Source Code');
  });
  ...
  test('Load initial XML via XMLAPI', async ({ page }) => {
    ...
    await expect(codeEditor).toContainText('りんごの選び方', { timeout: 10000 });
    ...
    await expect(wysiwyg).toContainText('自分にぴったりのりんごを選ぶ', { timeout: 10000 });
  });
  ...
  test('Implement Code Editor and onChange binding', async ({ page }) => {
    ...
    await expect(codeEditorContent.locator('span.syntax-tag').first()).toBeVisible({ timeout: 10000 });
