...
<root>
  ...
  it("preserves formatting exactly during localized attribute update", () => {
    ...
    const expected = complexXml.replace('<section id="1">', '<section id="1" status="active">');
  ...
  it("preserves formatting exactly during text update", () => {
    ...
    const expected = complexXml.replace('<title>Section 1</title>', '<title>New Title</title>');
