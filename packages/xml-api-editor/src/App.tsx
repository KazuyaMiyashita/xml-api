...
function App() {
  ...
  const addLog = useCallback((event: ChangeEvent) => {
    ...
    setEventLogs(prev => [...prev.slice(-19), { timestamp, type: event.type, detail }]);
  }, []);
      ...
      try {
        ...
        xmlApi.on((event: ChangeEvent) => {
          setEventLogs(prev => {
            ...
            else if (event.type === 'structure') detail = `Structure changed at ${event.target?.getType()} (${event.target?.id})`;
            else if (event.type === 'attribute') detail = `Attribute changed: ${event.key} = ${event.newValue} on ${event.target?.getType()}`;
            else if (event.type === 'text') detail = `Text changed on ${event.target?.getType()} (${event.target?.id})`;
            
            return [...prev.slice(-19), { timestamp, type: event.type, detail }];
          });
  ...
  return (
    <div className="app-container">
      <header className="app-header">
        <h1>XML API Editor <span style={{fontSize: '0.6em', fontWeight: 'normal', opacity: 0.8, marginLeft: '10px'}}>Type: {docTypeInfo.type}</span></h1>
      </header>
      <main className="main-content">
        <div className="pane pane-left">
          <div className="pane-header">
            {docTypeInfo.type === DocumentType.MEI ? 'MEI View' : 'WYSIWYG Editor'}
          </div>
          ...
          <div className="pane-footer" style={{ height: '150px', borderTop: '1px solid #ccc', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div className="pane-header" style={{ background: '#eee', color: '#333' }}>Event Log</div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '5px', fontSize: '11px', fontFamily: 'monospace' }}>
              {eventLogs.map((log, i) => (
                <div key={i} style={{ marginBottom: '2px' }}>
                  <span style={{ color: '#888' }}>[{log.timestamp}]</span> 
                  <strong style={{ color: '#007acc' }}> {log.type}</strong>: {log.detail}
                </div>
              ))}
              <div ref={logsEndRef} />
            </div>
        ...
        <div className="pane pane-right">
          ...
          <div className="pane-body">
            {error ? (
              ...
              <CodeEditor api={api} version={version} onChange={handleCodeChange} />
            ) : (
              <div>Loading...</div>
            )}
          </div>
